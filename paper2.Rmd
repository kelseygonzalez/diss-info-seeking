...
pacman::p_load(tidyverse, glue, extrafont, here, jtools, 
...
934 Adults in the United States and find that 



Knowing that Google
trends data only encompasses a small portion of the information-seeking done by
modern humans, it is important to investigate what leads individuals to search
for a question online using a search engine versus more traditional means of
information gathering, such as network activation (asking someone in their
social network for informational support). I predict that few people will
activate their social network for informational support in my chosen case of
Covid-19 vaccinations because of the social cost of doing so and that the
majority will have utilized an online search engine.
...
the relationship between the coefficients and information seeking.^[Figures in this paper were all created using the `r` packages jtools [@jtools] and ggplot2 [@wickham_etal; @wickham11] ].
In Table \@ref(tab:table-model-2), I disaggregate information seeking behaviors
by investigating the difference in proportions of each of the vehicles in 
\@ref(tab:model-2-chi2) and then by modelling a series of logistic regressions 
that predict the 5 different avenues of seeking information about Covid-19: 
from another person, from a doctor, on an online forum, on a social networking 
site, or using an online search tool.
...
model predicting Vaccination Outlook.  Figure \@ref(fig:plot-model-3) then illustrates
the relationship between the coefficients and vaccination outlook. 
...
find_p <- function(x, y){
  tab <- table(x, y)
  as.numeric(chisq.test(tab)["p.value"])
}


chi_squared_tests <- 
tribble(~col, ~Person, ~Dr, ~`Online Forum`, ~`Online Network`, ~`Online Search`,
'Person',   NA, find_p(chi$pers, chi$dr), find_p(chi$pers, chi$forum), find_p(chi$pers, chi$net), find_p(chi$pers, chi$search),
'Dr',     NA, NA, find_p(chi$dr, chi$forum), find_p(chi$dr, chi$net), find_p(chi$dr, chi$search),
'Online Forum',  NA, NA, NA, find_p(chi$forum, chi$net), find_p(chi$forum, chi$search),
'Online Network',    NA, NA,NA, NA, find_p(chi$net, chi$search),
'Online Search', NA, NA, NA, NA, NA)


chi_squared_tests %>%
  mutate(across(where(is.numeric), ~ sprintf("%.3f", round(., digits = 3)))) %>% 
  mutate(Person = cell_spec(Person, format = "html", bold = Person < 0.05),
       Dr = cell_spec(Dr, format = "html", bold = Dr < 0.05),
       `Online Forum` = cell_spec(`Online Forum`, format = "html", bold = `Online Forum` < 0.05),
       `Online Network` = cell_spec(`Online Network`, format = "html", bold = `Online Network` < 0.05),
       `Online Search` = cell_spec(`Online Search`, format = "html", bold = `Online Search` < 0.05)) %>% 
  kable(., escape = FALSE,
      digits=3,  
      caption = "Pearson's $\\chi^2$ test with Yates' continuity
	correction  between each Information Search Vehicle",
      col.names = c("",'Person', 'Dr', 'Online Forum', 'Online Network', 'Online Search')      ) %>% 
  kable_styling(latex_options = c("hold_position", "striped"),
                full_width = F) %>%
  column_spec(column = 1, bold = T) %>% 
  footnote(c("Bolded values are significant using $\\alpha = 0.05$, two-tailed test"))
```
...
the Covid-19 vaccine?" Results are displayed in Table \@ref(tab:table-model-2).
Because respondents could select as many responses as applicable, each response
is recorded as a dichotomous variable and modeling using logistic regression.
...
TODO interpret table \@ref(tab:model-2-chi2), the chi2 tests to see differences



The first model predicts whether or not a respondent sought information
intentionally about Covid-19 vaccinations by asking their doctor or another
health professional. First and foremost, I find that those who received
information about Covid-19 vaccinations were 
